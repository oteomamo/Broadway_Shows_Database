...
<div class="container22">
    ...
    <div class="container container2">
        <div class="form-container">
            <h2>Search Theaters by Number of Seats </h2>
            <form method="post">
            
                <div></div><div></div>
                <label for="min_nr">Min Number of Seats:</label>
                <input type="int" id="min_nr" name="min_nr" required>
                <br><br>
                <label for="max_nr">Max Number of Seats:</label>
                <input type="int" id="max_nr" name="max_nr" required>
                <br><br>
...
if (isset($_POST['show_table'])) {
    ...
    $query = "SELECT DISTINCT theater.Name, theater.Address, theater.Number_of_Seats
            FROM theater
            WHERE theater.Number_of_Seats BETWEEN '$min_nr' AND '$max_nr' ";
    $result = mysqli_query($conn, $query);
    echo "<div style='text-align: center;'>";
    echo "<table style='margin: 0 auto; border-collapse: collapse; border: 1px solid white;'>";
    echo "<tr style='background-color: hsl(194, 76%, 52%, 1);'><th style='border: 1px solid white; padding: 5px;'>Theater Name</th><th style='border: 1px solid white; padding: 5px;'>Address</th><th style='border: 1px solid white; padding: 5px;'>Number_of_Seats</th></tr>";
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr>";
        echo "<td style='background-color: hsl(194, 76%, 92%, 1); border: 1px solid white; padding: 5px;'>" . $row['Name'] . "</td>";
        echo "<td style='background-color: hsl(194, 76%, 92%, 1); border: 1px solid white; padding: 5px;'>" . $row['Address'] . "</td>";
        echo "<td style='background-color: hsl(194, 76%, 92%, 1); border: 1px solid white; padding: 5px;'>" . $row['Number_of_Seats'] . "</td>";
        echo "</tr>";
